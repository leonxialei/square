...
class Mayikami2 {
    ...
    private $_key = '4ab9f060cf05bec7097503330530b726';
    private $_url = 'http://103.101.204.161/';
    public function order($request) {
        $parameters = [
            ...
            'notify' => config('app.url').'/api/pay/mayikami2/notify',
            'time' => time(),
...
//        sign=md5(mchid+mch_order_id+price+paytype+notify+time+rand+商户秘钥)
        ...
        $sign = md5($this->_mchId.$parameters['mch_order_id'].
            $parameters['price']. $parameters['paytype'].
            $parameters['notify'].$parameters['time'].$parameters['rand']
            .$this->_key);
...
//        sign = md5（mchid + mch_order_id + pt_order_id + price + paytype + status + 商户秘钥）；
