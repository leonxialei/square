...
class Xiyou1 {
    private $_mchId = '9072443';
    private $_key = '7D04B9871309F892CD41F14E1505A542';
    private $_url = 'https://vip.x-ybn.com/';
    public function order($request) {



        $paykey = $this->_key;
        $url = 'https://vip.x-ybn.com/api/pay';
        $parameter = array(
            "api_id" => $this->_mchId,
            "orderid" => 'YZ' . $request['orderNo'],
            "money" => $request['amount']/100,
            "notify_url" =>config('app.url').'/api/pay/xiyou1/notify',
            "return_url" =>config('app.url').'/api/pay/xiyou1/notify',
        );
        ksort($parameter);
        reset($parameter);
        $fieldString = [];
