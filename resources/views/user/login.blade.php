...
<div class="main-body">
    <div class="login-main">
        ...
        <form class="layui-form login-bottom">
            <div class="center">
                <div class="item">
                    <input class="admin-input-username" type="text" name="username" lay-verify="required"  placeholder="请输入登录账号" maxlength="24"/>
                </div>

                <div class="item">
                    <input class="password-input" type="password" name="password" lay-verify="required"  placeholder="请输入密码" maxlength="20">
                    <span class="bind-password icon icon-4"></span>
                </div>

                <div id="validatePanel" class="item" style="width: 137px;">
                    <input id="refreshCaptcha" type="text" name="captcha" placeholder="不区分大小写" maxlength="5">
                    <img  onclick="refreshImg(this)" class="validateImg" src="{{$builder->inline()}}">
                </div>

            </div>

            <div class="layui-form-item" style="text-align:center; width:100%;height:100%;margin:0px;">
                <button type="button" class="login-btn admin-button" lay-submit="" lay-filter="login">立即登录</button>
            </div>
        </form>
    </div>
</div>
<div class="footer">
    ©版权所有 2019-2022 宇宙工作室<a target="_blank" href="http://www.miitbeian.gov.cn"></a>
</div>
<script src="{{secure_url('lib/jquery-3.4.1/jquery-3.4.1.min.js')}}" charset="utf-8"></script>
<script src="{{secure_url('lib/layui-v2.5.5/layui.js')}}" charset="utf-8"></script>

<script src="{{secure_url('lib/jq-module/jquery.particleground.min.js')}}" charset="utf-8"></script>
<script>
    ...
    layui.use(['jquery', 'layer'], function(){
        ...
        $('.admin-button').click(function(){
            ...
            var captcha = $('#refreshCaptcha').val();
            if(user == '') {
                layer.msg('账号不能为空！');
                return false;
            }
        ...
        i = 1;
        refreshImg = function(_this){

            var image = '{{secure_url('user/image')}}?'+i;
